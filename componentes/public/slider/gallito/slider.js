...
var SliderGallito = function () {
	function SliderGallito() {
		_classCallCheck(this, SliderGallito);

		this.confSlider();
		this.iniciar();
	}

	_createClass(SliderGallito, [{
		key: "confSlider",
    ...
    */
		...
		value: function iniciar() {
			...
			//le asignaremos el preload
			var alturaDeLaImagen = $(".contendor-img img").height();
			//$(".preload-img").css("height",alturaDeLaImagen+"px");


			$(".contendor-img img").load(function () {
				setTimeout(function () {
					$(".item").fadeIn("slow").show();
					$(".preload-img").addClass("hideDiv");
				}, 1500);
			});
		}
	}]);

	return SliderGallito;
}();

$(document).on("ready", inicio);
function inicio() {
	var obj = new SliderGallito();
}